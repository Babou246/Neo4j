...
def update(tx,name,prenom,telephone,email,adresse,profession,age,sexe,):
    tx.run(f"create (a:Friends {name: $name,prenom: $prenom,telephone: $telephone,email: $email,adresse:$adresse,profession :$profession,age: $age,sexe: $sexe,id:$id}) return a",name=name,prenom=prenom,telephone=telephone,email=email,adresse=adresse,profession=profession,age=age,sexe=sexe)

def deleted(tx,name):
    tx.run("match (a:Friends {name:$name}) DELETE a",name=name)

def create_insertion(tx,name,prenom,telephone,email,adresse,profession,age,sexe,name2,prenom2,email2,adresse2,profession2,age2,sexe2,relation,types):
    tx.run("""
        CREATE (a:PAPA {name: $name,prenom: $prenom,telephone: $telephone,email: $email,adresse:$adresse,profession :$profession,age: $age,sexe: $sexe})
        CREATE (b:MAMAN {name: $name2,prenom: $prenom2,telephone: $telephone2,email: $email2,adresse:$adresse2,profession :$profession2,age: $age2,sexe: $sexe2})
        CREATE (a)-[:$relation,{type:$types}]->(c)
        
    """,name,prenom,telephone,email,adresse,profession,age,sexe,name2,prenom2,email2,adresse2,profession2,age2,sexe2,relation,types)
...
def find_and_return_person(tx, person_name):
        ...
        return [record["prenom"] for record in result]
...
def creer_un_noeud():
    if request.method == "POST":
        ...
        try:
            with driver.session() as Session:
                Session.write_transaction(create_insertion,name,prenom,telephone,email,adresse,profession,age,sexe,name2,prenom2,email2,adresse2,telephone2,profession2,age2,sexe2,relation,types)
...
def admin():
    ...
    if 'name' in session:
        name = session['name']
    return render_template('index.html',data=data,countData=len(data),dico=dico,countFriends=len(dico),current_user = name)
